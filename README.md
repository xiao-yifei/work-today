...
- **换算**：按今日已赚换算能买多少件商品
- **日历**：当月格子。周末、法定节假日默认休息，调休补班默认上班；先点「编辑」再点日期才能改。日薪按本月上班天数计算
- **我的**：月薪、上下班、午休、备忘、商品单价。每月工作日不再手填，由日历算出
...
```
本月上班天数 = 日历上未标休息的天数（周末、法定假日默认休息，调休默认上班）
日薪         = 月薪 / 本月上班天数
有效工时     = 下班 − 上班 − 午休
秒薪         = 日薪 / 有效秒数
今日已赚     = 已工作秒数 × 秒薪
...
默认班次：月薪 `12223`，`09:00`–`19:00`，午休 `12:00`–`13:00`。节假日按国务院 2025、2026 年通知写在 `src/utils/holidays.ts`，改完请同步到 `mp/src/utils/holidays.ts`。

两套实现共用同一套算法。改根目录的 `src/utils/work.ts` / `holidays.ts` / `types.ts` 后，请同步到 `mp/src/`。
