...
- **今日**：工作状态、下班倒计时（下班或休息后改显示金额）、今日已赚、本月累计、已上班天数、进度条、时/分/秒薪、购买力（默认咖啡 ¥15、午餐 ¥35）、备忘、小荧陪你。先赚回不放这一页。打开「扣除支出」后，主卡、时/分/秒薪和购买力改按扣除后口径
- **换算**：三档「先赚回 / 今天能换 / 想买的」。先赚回按月固定支出摊到每个上班日，对照毛收入的今日已赚看覆盖和净赚，不跟「扣支出」开关走；今天能换、想买的跟开关走。要改先点编辑，点完成才保存
- **日历**：当月格子。休息制度（双休 / 单休 / 大小周）在这里改，观看时大小周会标出本周大/小周；法定节假日默认休息，调休补班默认上班；先点「编辑」再点日期才能改单日。累计卡只写本月累计和上班天数，不写今日已赚；「扣支出」打开时累计改成扣除后口径
- **我的**：月薪、上下班、午休。没保存过月薪时输入框空着。每月工作日不再手填，由日历算出。预览主数字是时薪，日薪写在下一行。扣除支出开关在表单最后一行；打开后今日、换算、日历、预览都跟它走
